...
INPUT_KEY_LABELS = 'labels'
INPUT_KEY_BOUNDING_BOX = 'bounding_boxes'
...
class BaseImageAugmentationLayer(base_layer.BaseRandomLayer):
  ...
  def call(self, inputs, training=True):
    ...
    if training:
      ...
      images = inputs[INPUT_KEY_IMAGES]
  ...
  def _augment(self, inputs):
    image = inputs.get(INPUT_KEY_IMAGES, None)
    label = inputs.get(INPUT_KEY_LABELS, None)
    bounding_box = inputs.get(INPUT_KEY_BOUNDING_BOX, None)
  ...
  def _format_inputs(self, inputs):
    if tf.is_tensor(inputs):
      ...
      return {INPUT_KEY_IMAGES: inputs}, False
  ...
  def _format_output(self, output, is_dict):
    if not is_dict:
      return output[INPUT_KEY_IMAGES]
    else:
      return output

  def _ensure_inputs_are_compute_dtype(self, inputs):
    if isinstance(inputs, dict):
      inputs[INPUT_KEY_IMAGES] = utils.ensure_tensor(inputs[INPUT_KEY_IMAGES],
                                             self.compute_dtype)
    else:
      inputs = utils.ensure_tensor(inputs, self.compute_dtype)
    return inputs
...
class RandomHeight(BaseImageAugmentationLayer):
  ...
  def _batch_augment(self, inputs):
    images = self.augment_image(inputs[INPUT_KEY_IMAGES], transformation=None)
    result = {INPUT_KEY_IMAGES: images}
...
class RandomWidth(BaseImageAugmentationLayer):
  ...
  def _batch_augment(self, inputs):
    images = self.augment_image(inputs[INPUT_KEY_IMAGES])
    result = {INPUT_KEY_IMAGES: images}
